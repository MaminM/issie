...
/// 
/// I have had to import some modules from WaveSimSelect for reference.
/// 
/// 
        ...
        |> List.map (fun wave ->
            tr [] [
                ...
                td [] [
                    input [
                        Type "checkbox"
                        Checked (ws.SelectedWaves.Contains wave.WaveId)
                        OnChange (fun _ -> dispatch (ToggleWave wave.WaveId))
                    ] []
                ]
            ]
        )
